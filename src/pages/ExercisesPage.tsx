...
export function ExercisesPage() {
    ...
    return (
      <div className="min-h-screen pb-24">
        <header className="px-4 pt-12 pb-4 safe-top">
          ...
          <h1 className="text-2xl font-bold text-slate-100">{selectedExercise.name}</h1>
        </header>

        <div className="px-4 space-y-6">
          {/* Tags */}
          <div className="flex gap-2 flex-wrap">
            <span className="px-3 py-1 rounded-full bg-emerald-600/20 text-emerald-400 text-sm">
          ...
          {/* Defaults */}
          <div className="p-4 rounded-xl bg-slate-800/50 border border-slate-700">
            <h3 className="text-sm font-medium text-slate-400 mb-3">Defaults</h3>
          ...
          {averages && (averages.avgWeight > 0 || averages.avgReps > 0) && (
            <div className="p-4 rounded-xl bg-emerald-600/10 border border-emerald-600/20">
              <h3 className="text-sm font-medium text-emerald-400 mb-3">Last Week Averages</h3>
              <div className="grid grid-cols-2 gap-4">
                {averages.avgWeight > 0 && (
                  <div>
                    <div className="text-2xl font-bold text-slate-100">{averages.avgWeight} lb</div>
                    <div className="text-sm text-slate-400">Avg Weight</div>
                  </div>
                )}
                {averages.avgReps > 0 && (
                  <div>
                    <div className="text-2xl font-bold text-slate-100">{averages.avgReps}</div>
                    <div className="text-sm text-slate-400">Avg Reps</div>
                  </div>
                )}
              </div>
            </div>
          ...
          {selectedExercise.alternatives && selectedExercise.alternatives.length > 0 && (
            <div className="p-4 rounded-xl bg-slate-800/50 border border-slate-700">
              <h3 className="text-sm font-medium text-slate-400 mb-3">Alternatives</h3>
                ...
                {selectedExercise.alternatives.map(altId => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full p-3 rounded-lg bg-slate-700/50 text-left hover:bg-slate-700 transition-colors"
                    >
  ...
  return (
    <div className="min-h-screen pb-24">
      <header className="px-4 pt-12 pb-4 safe-top">
        <h1 className="text-3xl font-bold text-slate-100">Exercises</h1>
        <p className="text-slate-400 mt-1">{exercises.length} exercises</p>
      </header>
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start">
        <div>
          <div className="font-medium text-slate-100">{exercise.name}</div>
